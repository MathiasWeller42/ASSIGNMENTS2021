...
func (peer *Peer) TakeNewConnection(listener net.Listener) {
	...
	peer.connections = (peer.connections, in_conn)
...
func (peer *Peer) ConnectToNetwork(uri string) net.Conn {
	...
	if err != nil {
		...
		peer.connectionsMutex.Lock()
		peer.connections = append(peer.connections, out_conn)
		peer.connectionsMutex.Unlock()
...
func (peer *Peer) SendMessages() {
	for {
		...
		if !peer.messagesSent[message] {
			...
			fmt.Println("Message: " + message) //!!! Update ledger
...
func (peer *Peer) UpdateLedger(transaction *Transaction) {
	//!!! Check om accounten findes ellers sæt den ind først
...
func (peer *Peer) MarshalConnections() []byte {
	peer.connectionsMutex.Lock()
	bytes, err := json.Marshal(peer.connections)
	peer.connectionsMutex.Unlock()
	if err != nil {
		fmt.Println("Marshaling failed")
	}
...
func (peer *Peer) DemarshalConnections([]byte) []net.Conn {
