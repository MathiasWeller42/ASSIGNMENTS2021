...
let errorNumberFunctionArguments id formals args = 
  "wrong number of arguments supplied for " ^ string_of_symbol id ^ ". Expected " ^ (string_of_int (List.length formals)) ^ " arguments, but got " ^ (string_of_int (List.length args))
...
let errorAssignmentTypesShouldMatch declaredType initializedType  = 
  "Expected type " ^ (string_of_type declaredType) ^ " in assigment. got: " ^ (string_of_type initializedType)

let errorValDeclTypesShouldMatch declaredType initializedType  = 
"Expected type " ^ declaredType ^ " in assigment. got: " ^ initializedType
