    ...
    def compute_w_log_opt(self, X): 
        ...
        for col in range(1,n): #For each column in omega
            for row in range(K): #For each row (each z-state) in omega
                ...
                for z_i in allTransitions[row]: #For each (applicable) row in previous column
...
if __name__ == '__main__':
    ...
    size = 0 #this means read all the data
    ...
    for i in range(1, 6): #The genome number we validate on
        print("Starting training - validation set is " + str(i))
        ...
        #finish training (i.e. divide by the sums of rows)
    ...
    print("These are the best transprobs:")
    printRows(A_max)
    print("These are the best emissionprobs:")
    printRows(fi_max)
    ...
    for i in range(6,11):
        file.write("> pred-ann" + str(i) + "\n")

        #decode
        print("Decoding genome " + str(i))
    ...
    print("Done, results in fasta.fa")
